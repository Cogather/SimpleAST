...
class AnalysisResult:
    ...
    # ==================== 以下方法已废弃，由 FunctionReporter 使用 ====================
    # 保留是为了向后兼容，如果直接调用这些内部方法

    def _generate_recursive_function_info_DEPRECATED(self, func_name: str, lines: List[str], number_prefix: str, visited: Set[str], all_data_structures: Set[str], all_external_funcs: Set[str]):
        """已废弃：请使用 FunctionReporter"""
        pass  # 保留空壳，避免报错
    ...
    def _generate_recursive_function_info(self, func_name: str, lines: List[str], number_prefix: str, visited: Set[str], all_data_structures: Set[str], all_external_funcs: Set[str]):
        """递归生成函数信息（带序号层级） - 内部使用"""
        # 注意：这个方法现在只是为了避免破坏性变更而保留
        # 实际逻辑已移到 FunctionReporter
        pass  # 让调用者使用新的 FunctionReporter

    def _OLD_generate_recursive_function_info(self, func_name: str, lines: List[str], number_prefix: str, visited: Set[str], all_data_structures: Set[str], all_external_funcs: Set[str]):
        """原始实现（已废弃，保留作为参考）"""
    ...
    def _generate_recursive_function_info(self, func_name: str, lines: List[str], number_prefix: str, visited: Set[str], all_data_structures: Set[str], all_external_funcs: Set[str]):
        ...
        if direct_external_deps:
            ...
            logger.info(f"{indent}[Mock生成] 外部函数分类: 业务{len(classified.get('business', []))}个, 标准库{len(classified.get('standard_lib', []))}个, 日志{len(classified.get('logging', []))}个")
    ...
    def _extract_data_structures_from_single_function(self, func_name: str):
        ...
        logger.info(f"[数据结构提取] 正则提取: {len(type_matches)} 个参数类型, {len(class_matches)} 个类名")
        if all_types:
        ...
        for type_name in all_types:
            ...
            if is_basic_type:
                logger.info(f"[数据结构提取] ✗ 过滤项目基础类型: {type_name} (匹配: {matched_pattern})")
                external_filtered += 1
                continue
    ...
    def _extract_constants_from_function(self, func_name: str) -> Dict[str, str]:
        ...
        logger.info(f"[常量提取] 准备搜索 {min(len(possible_headers), 50)} 个文件 (共找到 {len(possible_headers)} 个)")
